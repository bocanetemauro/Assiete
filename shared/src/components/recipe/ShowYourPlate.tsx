...
import { createPost, socialError } from "@/lib/social";
    ...
    try {
      ...
      if (entry) {
        ...
        if (share && user.username) {
          try {
            await createPost({ authorId: user.id, body: [title.trim(), note.trim()].filter(Boolean).join(" — "), photo, recipeId: recipe.id });
            setShared(true);
            toast({ title: "Gedeeld met je vrienden", tone: "success", action: { label: "Bekijk feed", href: "/vrienden" } });
          } catch (err) {
            toast({ title: "Delen met vrienden lukte niet", description: socialError(err, "Je bord is wel bewaard in Mijn keuken.") });
          }
        }
      }
    } finally {
            ...
            {saved ? (
              <motion.div key="saved" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.7, ease: EASE_CHEF }} className="flex flex-col items-center">
                ...
                <p className="mt-8 flex items-center gap-2 text-[15px] text-ivory/70">
                  ...
                  {shared ? "Opgeslagen en gedeeld met je vrienden" : "Opgeslagen in je persoonlijke bibliotheek"}
                </p>
              ...
              <motion.form key="form" onSubmit={submit} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="space-y-5 rounded-[32px] border border-ivory/10 bg-ivory/[0.03] p-5 sm:p-7">
                ...
                {ready && !user ? (
                  ...
                  <>
                    {user?.username && (
                      <label className="flex cursor-pointer items-start gap-3 rounded-2xl bg-ivory/[0.05] p-4">
                        <input type="checkbox" checked={share} onChange={(e) => setShare(e.target.checked)} className="mt-0.5 size-5 shrink-0 accent-[var(--color-brass)]" />
                        <span className="min-w-0">
                          <span className="flex items-center gap-2 text-[14.5px] font-semibold text-ivory">
                            <Users className="size-4 text-brass-soft" /> Deel ook met je vrienden
                          </span>
                          <span className="mt-0.5 block text-[13px] text-ivory/55">Alleen jij en je vrienden zien deze post in de feed.</span>
                        </span>
                      </label>
                    )}
                    <Button type="submit" variant="brass" size="lg" className="w-full" disabled={!ready || busy}>
                      {busy ? "Bezig met opslaan…" : share && user?.username ? "Bewaar en deel" : "Bewaar in Mijn keuken"}
                    </Button>
                  </>
                )}
              </motion.form>
