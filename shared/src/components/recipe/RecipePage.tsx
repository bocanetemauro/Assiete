      ...
      <div className="container-page relative grid gap-10 pb-14 pt-8 lg:grid-cols-[1fr_1.02fr] lg:items-center lg:gap-14 lg:pb-24 lg:pt-12">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: EASE_CHEF }}>
          <nav className="flex items-center gap-2 text-[13px] text-muted" aria-label="Kruimelpad">
            <Link href="/recepten" className="-my-2 inline-flex min-h-11 items-center gap-1.5 py-2 font-semibold text-ink hover:text-brass sm:my-0 sm:min-h-0 sm:py-0">
              <ArrowLeft className="size-3.5" /> Recepten
            </Link>
            {primaryCategory && (
              <>
                <span>/</span>
                <Link href={`/recepten?categorie=${primaryCategory}`} className="-my-2 inline-flex min-h-11 items-center py-2 hover:text-ink sm:my-0 sm:min-h-0 sm:py-0">
        ...
        <nav className="no-scrollbar -mx-2 flex gap-1 overflow-x-auto" aria-label="Secties van het recept">
          {SECTIONS.map((s) => (
            <a
              ...
              href={`#${s.id}`}
              className={cn(
                "relative inline-flex min-h-11 shrink-0 items-center rounded-full px-3.5 py-2 text-[13.5px] font-semibold transition-colors sm:min-h-0",
    ...
    <article>
      ...
      <section id="ingredienten" className="scroll-mt-32 py-20 lg:py-28">
        <div className="container-page grid gap-14 lg:grid-cols-[1.15fr_1fr] lg:gap-20">
          ...
          {/* Rechterkolom: alles wát je nodig hebt en waaróm — zo blijft links de werklijst en staat rechts geen lege ruimte. */}
          <aside className="space-y-6">
            ...
            {/* Naast elkaar zolang de kolom breed is; als rail onder elkaar, dat leest rustiger. */}
            ...
            {recipe.equipment.length > 0 && (
              <Reveal delay={0.2}>
                <div className="rounded-[26px] border border-line bg-cream p-6">
                    ...
                    {recipe.tags.map((tag) => (
                      <li key={tag}>
                        <Link
                          ...
                          className="inline-flex h-11 items-center rounded-full border border-line px-4 text-[13.5px] text-ink-soft transition hover:border-brass hover:text-ink sm:h-9"
                        >
                          {tag}
                        </Link>
                      </li>
