          ...
          <div className="grid grid-cols-2 gap-10 sm:grid-cols-3">
            {COLUMNS.map((col) => (
              <div key={col.title}>
                ...
                <ul className="mt-3 space-y-0.5 sm:mt-5 sm:space-y-3.5">
                  {col.links.map((link) => (
                    <li key={link.href}>
                      {/* Ruimer aanraakvlak op de telefoon; op groter scherm blijft de compacte lijst. */}
                      <Link href={link.href} className="inline-flex min-h-11 items-center text-[15px] text-ivory/80 transition-colors hover:text-brass-soft sm:min-h-0">
                        {link.label}
                      </Link>
                    </li>
                  ))}
