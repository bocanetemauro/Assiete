...
export function MobileNav() {
  ...
  return (
    <nav
      className="fixed inset-x-1.5 bottom-3 z-50 rounded-[26px] border border-line/80 bg-ivory/90 px-1 pb-[max(0.35rem,env(safe-area-inset-bottom))] pt-1.5 shadow-float backdrop-blur-xl sm:inset-x-3 sm:px-1.5 lg:hidden"
          ...
          return (
            <li key={href} className="min-w-0">
              <Link href={href} aria-current={active ? "page" : undefined} className="relative flex min-h-14 flex-col items-center justify-center gap-1 rounded-2xl text-[10.5px] font-semibold">
                ...
                {!accent && <span className={cn("relative w-full truncate px-0.5 text-center", active ? "text-ink" : "text-muted")}>{label}</span>}
