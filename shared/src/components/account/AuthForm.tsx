...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (usernameStatus === "taken") {
      setBusy(false);
      setError("Deze username is al bezet. Kies een andere.");
      return;
    }
            ...
            <>
              ...
              <form onSubmit={submit} className="mt-10 space-y-5" noValidate>
                {!login && (
                  <>
                    <div>
                      <Label htmlFor="name" hint="Zo zien anderen je">
                        Toonbare naam
                      </Label>
                      <Input id="name" autoComplete="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="bijv. Mauro" maxLength={80} required />
                    </div>
              ...
              {login && (
                <p className="mt-5 text-center text-[14px]">
                  <Link href="/wachtwoord-vergeten" className="-my-1.5 inline-flex min-h-11 items-center py-1.5 sm:my-0 sm:min-h-0 sm:py-0 font-semibold text-ink underline-offset-4 hover:underline">
                    Wachtwoord vergeten?
                  </Link>
                </p>
              )}
              ...
              <p className="mt-8 text-center text-[14px] text-muted">
                ...
                <Link href={switchHref} className="-my-1.5 inline-flex min-h-11 items-center py-1.5 sm:my-0 sm:min-h-0 sm:py-0 font-semibold text-ink underline-offset-4 hover:underline">
                  {login ? "Maak er een aan" : "Log in"}
                </Link>
              </p>
              <p className="mt-6 text-center text-[12px] leading-relaxed text-muted/80">
                Een account is optioneel:{" "}
                <Link href="/recepten" className="-my-1.5 inline-flex min-h-11 items-center py-1.5 sm:my-0 sm:min-h-0 sm:py-0 underline underline-offset-2">
                  alle recepten
                </Link>{" "}
