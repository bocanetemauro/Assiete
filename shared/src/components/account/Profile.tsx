...
import { ArrowRight, AtSign, Camera, Check, Flame, LogOut, Plus, Repeat, Users } from "lucide-react";
...
import { UsernameField, type UsernameStatus } from "@/components/social/UsernameField";
import { memberHref } from "@/components/social/parts";
    ...
    <form
      ...
      onSubmit={async (e) => {
        ...
        if (usernameChanged && usernameStatus === "taken") {
          toast({ title: "Opslaan mislukt", description: "Deze username is al bezet. Kies een andere." });
          return;
        }
        setSaving(true);
        const result = await updateProfile({ name, bio: bio.trim() || null, ...(usernameChanged ? { username } : {}) });
        setSaving(false);
        toast(result.ok ? { title: "Profiel bijgewerkt", tone: "success" } : { title: "Opslaan mislukt", description: result.error });
      }}
    >
      <div>
        <Label htmlFor="profile-name" hint="zo zien anderen je">
          Toonbare naam
        </Label>
        <Input id="profile-name" value={name} onChange={(e) => setName(e.target.value)} maxLength={60} autoComplete="name" required />
      </div>
      <UsernameField id="profile-username" value={username} onChange={setUsername} current={user.username} onStatus={setUsernameStatus} />
      <div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button type="submit" size="lg" disabled={saving || (usernameChanged && (usernameStatus === "taken" || usernameStatus === "invalid" || !username))}>
        ...
        <div className="container-page flex flex-col gap-8 pb-10 pt-12 md:flex-row md:items-center md:justify-between lg:pb-14 lg:pt-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: EASE_CHEF }} className="flex items-center gap-5 sm:gap-8">
            ...
            <div className="min-w-0">
              ...
              <h1 className="mt-2 break-words font-serif text-[clamp(2.6rem,6vw,4.8rem)] leading-[0.95]">{user.name}</h1>
              {user.username ? (
                <Link href={memberHref(user.username)} className="mt-2 inline-block text-[16px] font-semibold text-brass hover:underline">
                  @{user.username}
                </Link>
              ) : (
                <button type="button" onClick={() => setTab("instellingen")} className="mt-2 inline-flex items-center gap-1.5 text-[14px] font-semibold text-bordeaux hover:underline">
                  <AtSign className="size-4" /> Kies een username
                </button>
              )}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <ButtonLink href="/vrienden" variant="secondary">
              <Users className="size-4" /> Vrienden
            </ButtonLink>
            <Button
              variant="ghost"
              onClick={async () => {
