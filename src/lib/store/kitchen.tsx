...
export interface KitchenContextValue {
  ...
  updateProfile: (patch: { name?: string; bio?: string | null; avatarUrl?: string | null }) => Promise<ActionResult>;
...
export function KitchenProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const readProfile = async (authUser: { id: string; email?: string | null; created_at?: string; email_confirmed_at?: string | null }) => {
      const { data } = await supabase.from("profiles").select("display_name, bio, avatar_url").eq("id", authUser.id).maybeSingle();
  ...
  const signUp = useCallback(
    async ({ name, email, password }: { name: string; email: string; password: string }): Promise<SignUpResult> => {
      ...
      const { data, error: e } = await supabase.auth.signUp({
        email: email.trim(),
        password,
        options: { data: { display_name: name.trim() }, emailRedirectTo: redirectTo("/profiel?welkom=1") },
      });
  ...
  const updateProfile = useCallback(
    async (patch: { name?: string; bio?: string | null; avatarUrl?: string | null }): Promise<ActionResult> => {
      if (!supabase || !user) return { ok: false, error: "Log eerst in." };
      const row: Record<string, string | null> = {};
      if (patch.name !== undefined) row.display_name = patch.name.trim().slice(0, 80) || "Chef";
