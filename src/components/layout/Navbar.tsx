...
export function Logo({ light = false }: { light?: boolean }) {
  return (
    <Link href="/" className="group flex items-center gap-3" aria-label={`${BRAND.name} — home`}>
      <span className={cn("grid size-10 place-items-center rounded-full font-serif text-[22px] italic transition-transform duration-500 group-hover:rotate-[-12deg]", light ? "bg-ivory text-ink" : "bg-ink text-ivory")}>
        A
      </span>
      <span className="leading-none">
        <span className={cn("block font-serif text-[26px] font-medium tracking-tight", light ? "text-ivory" : "text-ink")}>{BRAND.name}</span>
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={cn("relative rounded-full px-4 py-2.5 text-[14px] font-medium transition-colors duration-300", active ? "text-ink" : "text-muted hover:text-ink")}
                >
                  {item.label}
          ...
          <div className="flex items-center gap-1.5">
            ...
            {ready && user ? (
              <Link href="/profiel" className="hidden items-center gap-2.5 rounded-full border border-line bg-cream/70 py-1 pl-1 pr-4 transition hover:border-ink/25 sm:flex">
                ...
                <span className="text-[13.5px] font-semibold">{user.name.split(" ")[0]}</span>
              </Link>
            ) : (
              <ButtonLink href="/inloggen" size="sm" className="hidden sm:inline-flex">
                Inloggen
              </ButtonLink>
            )}
            <button
