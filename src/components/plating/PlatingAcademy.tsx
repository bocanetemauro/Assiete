          ...
          <div className="mt-12 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            {PLATFORM_RECIPES.map((r, i) => (
              <Reveal key={r.id} delay={(i % 4) * 0.05}>
                <Link href={`/recepten/${r.slug}#plating`} className="group block rounded-[28px] p-4 transition-colors hover:bg-cream">
                  ...
                  <p className="mt-4 font-serif text-[1.35rem] leading-tight">{r.title}</p>
                </Link>
              </Reveal>
            ))}
          </div>
